...
- Single test: `zig build test --test-filter "test name"` or use
  `zig test src/main.zig --test-filter "name"`

## Code Style (Zig 0.15.2)

- Imports: `std` first, then `builtin`, then project imports; group with blank
  lines
- Types: Use semantic type aliases (`ByteLength`, `WasmPtr`, `ConfigId`) over
  raw primitives
- Naming: `snake_case` functions/vars, `PascalCase` types, `SCREAMING_CASE`
  comptime constants
- Enums: Use Zig enum syntax with explicit backing type when needed (e.g.,
  `enum(u32)`)
- Errors: Return `!T` for fallible functions; use `catch` with explicit handling
- Comments: Use `///` for doc comments, `//` for inline; add section headers
  with `// ===`
- Comptime: Prefer comptime evaluation for static data (JSON metadata, embedded
  files)
- Wasm: Use `builtin.cpu.arch == .wasm32` for conditional compilation
- Tests: Place tests at end of file with `test "descriptive name"` blocks;
  native-only
